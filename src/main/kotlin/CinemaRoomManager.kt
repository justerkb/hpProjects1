...
fun main() {
    ...
    val shablon: MutableList<MutableList<String>> = MutableList(row){ MutableList(places) {"S "} }
    var tickets = 0
    var money = 0
    val totalIncome = if(row % 2 == 0) ((row / 2) * 10 * places) + ((row / 2) * 8 * places ) else {
        ((row / 2) * 10 * places) + ((row / 2 + 1) * 8 * places )
    }
    fun statistics() {
        println("Number of purchased tickets: $tickets")
        println("Percentage: " + "%.2f".format((tickets * 100) / (row * places).toDouble()) + "%")
        println("Current income: $${money}")
        println("Total income: $${totalIncome}")
    }
    fun banner() {
        println("Cinema: ")
        println("  1 2 3 4 5 6 7 8 9 ".substringBefore(places.toString()) + "$places")
    ...
    fun buyTicket() {
        do {
            ...
            val places2 = readLine()!!.toInt()
            println()

            if (row2 > row || places2 > places) {
                println("Wrong input!")
            } else if(shablon[row2 - 1][places2 - 1] == "B ") {
                println("That ticket has already been purchased!")
            } else if (row * places <= 60 || row2 <= row / 2) {
                print("Ticket price: $10")
                shablon[row2 - 1][places2 - 1] = "B "
                tickets++
                money += 10
                break
            } else {
    ...
    """.trimIndent()
        ...
        val numberChose = readLine()!!.toInt()
        when(numberChose) {
            1 -> banner()
            2 -> buyTicket()
            3 -> statistics()
